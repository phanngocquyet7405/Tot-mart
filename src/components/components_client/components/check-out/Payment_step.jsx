...
function PaymentMethodCard({ method, selected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      {disabled && (
        <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 border border-slate-200 rounded-full px-2 py-1 shrink-0">
          Sắp ra mắt
        </span>
      )}
    </label>
...
export function PaymentStep({
  ...
  finalTotal,
  submitting,
  paymentStatus, // "idle" | "redirecting" | "failed" — chỉ có ý nghĩa với vnpay
  onApplyCoupon,
  onBack,
  onPlaceOrder,
}) {
